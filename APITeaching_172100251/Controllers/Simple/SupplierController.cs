...
namespace APITeaching_172100251.Controllers.Simple
{
    ...
    public class SupplierController : ControllerBase
    {
        ...
        {
            var entityList = await _supplierService.GetsAsync();
            if (entityList != null)
            {
        ...
        {
            int maxId = await _supplierService.GetsAsync()
            ...
            if (await _supplierService.CreateAsync(newModel) != null)
                return Ok(dto);
        ...
        {
            if (_supplierService.CheckExists(dto.Id))
            {
                var entity = new Supplier();
                _mapper.Map(dto, entity);
                if (await _supplierService.UpdateAsync(entity) != null)
                    return Ok(dto);
            }
            return NotFound();
        }
